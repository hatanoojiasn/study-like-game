...
[warn] quest=Q14 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- Q13のトークン定義を確認し、最小文法（文/式）を2〜3個に絞る。


### 8.2 編集してよいファイル（推奨）
- `src/engine/engine.c`（内部構文解析器）
- 関連テスト
...
- 推奨: 低リスク案。Q14 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 正常系3ケース/異常系3ケースをテーブル化
  - エラー位置情報を検証

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q14` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（小さな文法を定義しASTへ変換する。）が、次の難易度段差を1段下げます。

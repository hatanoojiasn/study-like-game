...
[warn] quest=Q09 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/game/game.c` の位置更新後に衝突判定を入れる位置を決める。


### 8.2 編集してよいファイル（推奨）
- `src/game/game.c`
- `tests/test_game_determinism.c`（衝突ケース追加）
...
- 推奨: 低リスク案。Q09 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 境界侵入しないことを1000ステップで確認
  - 衝突あり/なしケースの期待座標をアサート

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q09` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（AABB判定を導入し衝突有無を確定する。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q08 anomaly_detected=true
```
...
## 3. 目標
- ジョブを小片化し、複数フレームに分散実行する。
- 公開API（`platform.h` / `game.h` / `engine.h`）を変更せずに達成する。

## 4. 達成報酬
- 解放能力（アンロック）: **フレーム安定化II解放**
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/engine/engine.c` に長い処理候補のダミー負荷点を設計する。


### 8.2 編集してよいファイル（推奨）
- `src/engine/engine.c`
- 必要なら `src/main.c` の計測表示


### 8.3 できれば触らないファイル（理由付き）
- 外部キューライブラリ導入（学習意図から逸脱）


### 8.4 実装アプローチの選択肢
- 低リスク案: 先にログ観測点を増やし、次に挙動変更する。  
- 高速案: 先に挙動を直し、後で計測を追加する。  
- 推奨: 低リスク案。Q08 は再現性の担保がクリア速度に直結するため。
...
- ログの見方: `[timing]` の avg/min/max と `Q08` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（ジョブを小片化し、複数フレームに分散実行する。）が、次の難易度段差を1段下げます。

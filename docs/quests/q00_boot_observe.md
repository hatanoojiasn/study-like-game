...
## 1. 症状
- 観測現象: 起動直後に世界の状態が読めず、異常の再現も記録もできない。
- 再現手順:
  1. 既定ビルドを起動する。
...
[warn] quest=Q00 anomaly_detected=true
```
...
## 3. 目標
- ログHUD（FPS/dt/座標）と再現手順メモを整備する。
- 公開API（`platform.h` / `game.h` / `engine.h`）を変更せずに達成する。

## 4. 達成報酬
- 解放能力（アンロック）: **観測コンソール解放**
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `docs/README.md` の実行手順を確認する。
- `src/main.c` のHUD表示と1秒統計出力位置を特定する。


### 8.2 編集してよいファイル（推奨）
- `src/main.c`
- `src/game/game.c`（座標表示補助が必要な場合）


### 8.3 できれば触らないファイル（理由付き）
- `src/platform/platform.h`
- `src/game/game.h`
- `src/engine/engine.h`（公開API固定）


### 8.4 実装アプローチの選択肢
- 低リスク案: 先にログ観測点を増やし、次に挙動変更する。  
- 高速案: 先に挙動を直し、後で計測を追加する。  
- 推奨: 低リスク案。Q00 は再現性の担保がクリア速度に直結するため。
...
- 自動テスト（出発点）:
  ...
  - GUI確認: `./build/study_like_game` でHUD数値を目視確認

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q00` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（ログHUD（FPS/dt/座標）と再現手順メモを整備する。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q19 anomaly_detected=true
```
...
- 推奨: 低リスク案。Q19 は再現性の担保がクリア速度に直結するため。
...
- ログの見方: `[timing]` の avg/min/max と `Q19` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（保存量ログを追加し劣化を可視化する。）が、次の難易度段差を1段下げます。

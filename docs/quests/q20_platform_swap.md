...
[warn] quest=Q20 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/platform/platform.h` を読み、依存が漏れていないか監査する。
- `src/platform/host_raylib/platform_host.c` を代替できる粒度で整理する。
...
- 推奨: 低リスク案。Q20 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - HOST実装と代替実装で同じゲーム更新が動くことを確認
  - ヘッドレステストの継続成功

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q20` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（固定APIのまま時間/入力/描画実装を交換可能にする。）が、次の難易度段差を1段下げます。

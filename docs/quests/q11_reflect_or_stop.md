...
[warn] quest=Q11 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- 反射/停止どちらを採用するか `docs/roadmap.md` と整合する方針を決める。


### 8.2 編集してよいファイル（推奨）
- `src/game/game.c`
- 必要に応じて `docs/README.md` の挙動説明
...
- 推奨: 低リスク案。Q11 は再現性の担保がクリア速度に直結するため。
...
- ログの見方: `[timing]` の avg/min/max と `Q11` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（反射または停止の仕様を明文化して実装を揃える。）が、次の難易度段差を1段下げます。

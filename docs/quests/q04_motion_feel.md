...
## 1. 症状
- 観測現象: 移動がON/OFFだけで機械的、学習モチベーションが下がる。
- 再現手順:
  1. 既定ビルドを起動する。
...
[warn] quest=Q04 anomaly_detected=true
```
...
## 3. 目標
- 加速度・減衰の内部パラメータを導入して手触りを改善する。
- 公開API（`platform.h` / `game.h` / `engine.h`）を変更せずに達成する。

## 4. 達成報酬
- 解放能力（アンロック）: **チューニング機能解放**
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/game/game.c` の速度計算を読む。
- 現在の `speed_px_per_s` 定数がどこで効いているか確認する。
...
- 推奨: 低リスク案。Q04 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 固定入力（右押下2秒→離す2秒）で速度変化をログ化
  - `ctest --test-dir build-test --output-on-failure`

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q04` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（加速度・減衰の内部パラメータを導入して手触りを改善する。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q12 anomaly_detected=true
```
...
## 3. 目標
- 固定dt長時間試験と比較ログで決定性を監査する。
- 公開API（`platform.h` / `game.h` / `engine.h`）を変更せずに達成する。

## 4. 達成報酬
- 解放能力（アンロック）: **再現性保証バッジ解放**
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `tests/test_game_determinism.c` を拡張し、ステップ数とシナリオを増やす。


### 8.2 編集してよいファイル（推奨）
- `tests/test_game_determinism.c`
- `src/game/game.c`（決定性を壊す要因除去）


### 8.3 できれば触らないファイル（理由付き）
- 時刻由来の非決定処理を `game_step` に混ぜる


### 8.4 実装アプローチの選択肢
- 低リスク案: 先にログ観測点を増やし、次に挙動変更する。  
- 高速案: 先に挙動を直し、後で計測を追加する。  
- 推奨: 低リスク案。Q12 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 固定dt=16msで1000ステップ×2回の一致確認
  - CIで安定して通ること

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q12` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（固定dt長時間試験と比較ログで決定性を監査する。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q15 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- 命令セットを5〜10命令に制限して設計する。


### 8.2 編集してよいファイル（推奨）
- `src/engine/engine.c`（内部コード生成）
- 関連テスト
...
- 推奨: 低リスク案。Q15 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 入力AST→期待バイトコード列の一致確認
  - 命令数上限の境界テスト

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q15` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（ASTから固定長命令列へ変換する。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q07 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/main.c` のメインループを「入力」「更新」「描画」「統計」に分けてメモする。
- `src/engine/engine.c` に内部状態を持たせられるか検討する。
...
- 推奨: 低リスク案。Q07 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 60秒実行でmax dtとフレーム落ち率を改修前後比較
  - `ctest --test-dir build-test --output-on-failure`

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q07` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（入力/更新/監視を協調タスクへ分割する。）が、次の難易度段差を1段下げます。

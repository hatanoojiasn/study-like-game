...
[warn] quest=Q18 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/game/game.c` の位置更新式を抜き出し、固定dt/可変dt比較ログを設計する。
...
- 推奨: 低リスク案。Q18 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 同一初期条件で積分法ごとの誤差推移を出力
  - 5分実行で発散しない

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q18` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（Euler系の差分を観測し安定条件を導く。）が、次の難易度段差を1段下げます。

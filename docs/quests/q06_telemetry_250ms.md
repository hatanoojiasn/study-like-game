...
[warn] quest=Q06 anomaly_detected=true
```
...
## 3. 目標
- 250ms周期の内部タスクとしてテレメトリ送信を安定化する。
- 公開API（`platform.h` / `game.h` / `engine.h`）を変更せずに達成する。

## 4. 達成報酬
- 解放能力（アンロック）: **監視ネットワーク解放**
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/main.c` の1秒統計出力を基点に、250ms窓へ分解できる場所を探す。


### 8.2 編集してよいファイル（推奨）
- `src/main.c` または `src/engine/engine.c`（内部タスク化する場合）
...
- 推奨: 低リスク案。Q06 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - 250ms周期ログの誤差を60秒収集し平均誤差を算出
  - `ctest --test-dir build-test --output-on-failure`

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q06` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（250ms周期の内部タスクとしてテレメトリ送信を安定化する。）が、次の難易度段差を1段下げます。

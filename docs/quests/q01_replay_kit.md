...
## 1. 症状
- 観測現象: 同じ操作をしても毎回結果が違い、バグ報告が会話で終わる。
- 再現手順:
  1. 既定ビルドを起動する。
...
[warn] quest=Q01 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `tests/test_game_determinism.c` を読み、入力列の作り方を把握する。
- `src/engine/engine.c` から `engine_step` 呼び出し流れを追う。


### 8.2 編集してよいファイル（推奨）
- `tests/test_game_determinism.c`
- `src/main.c`（入力記録/再生トグルを内部追加する場合）
- `src/game/game.c`
...
- 推奨: 低リスク案。Q01 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - `ctest --test-dir build-test --output-on-failure -R test_game_determinism`
  - 同一入力列を2回流し、終端座標一致をログ比較

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q01` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（固定入力列を保存して再生し、再現可能な調査線を作る。）が、次の難易度段差を1段下げます。

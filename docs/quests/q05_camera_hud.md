...
## 1. 症状
- 観測現象: 画面端で情報が欠落し、異常時の原因追跡に時間がかかる。
- 再現手順:
  1. 既定ビルドを起動する。
...
[warn] quest=Q05 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- `src/main.c` と `src/game/game.c` の `platform_draw_text` 呼び出し箇所を列挙する。
...
- 推奨: 低リスク案。Q05 は再現性の担保がクリア速度に直結するため。

## 9. テスト計画
- 自動テスト（出発点）:
  - HUDにFPS/dt/座標/任意メトリクスが重ならず表示されることを目視
  - 5分放置でクラッシュしない

- 手動テスト: 60秒操作 + 60秒放置を1セットとして3回実施。  
- ログの見方: `[timing]` の avg/min/max と `Q05` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（HUDレイヤを整理し、メトリクスを常時確認できるようにする。）が、次の難易度段差を1段下げます。

...
[warn] quest=Q16 anomaly_detected=true
```
...
## 8. 実装の方向性
### 8.1 ここからスタート（最初の10分）
- Q15の命令セットに対し、実行ステップ関数の最小ループを設計する。


### 8.2 編集してよいファイル（推奨）
- `src/engine/engine.c`
- `src/main.c`（命令数メトリクス表示する場合）
...
- 推奨: 低リスク案。Q16 は再現性の担保がクリア速度に直結するため。
...
- ログの見方: `[timing]` の avg/min/max と `Q16` の警告ログ有無を比較する。

## 10. 次のクエストにどう繋がるか
この修理で得た安定性は次のQuestの前提装備になります。  
今回の介入（1フレーム命令上限と安全停止を設ける。）が、次の難易度段差を1段下げます。
